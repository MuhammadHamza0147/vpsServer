...
@section('content')
    <div class="page-container">
        ...
        <div class="modal fade" id="AddServer" tabindex="-1" role="dialog" aria-labelledby="mediumModalLabel"
            aria-hidden="true">
        ...
        <div class="modal fade" id="manipulate-server" tabindex="-1" role="dialog" aria-labelledby="mediumModalLabel"
            aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <div class="row ">
                            ...
                            <input type="hidden" id="server_id" name="server_id" class="form-control"
                                placeholder="Enter Server Name" required>

                            <input type="hidden" id="verifiedSessionId" name="verifiedSessionId" class="form-control"
                                placeholder="Enter Server Name" required>

                            <input type="hidden" id="secret" name="secret" class="form-control"
                                placeholder="Enter Server Name" required>
                            ...
                            <div class="col-sm-6" id="termination_reason_container" style="display: none;">
                                ...
                                <input type="text" id="termination_reason" name="termination_reason"
                                    class="form-control" placeholder="Enter Termination Reason">
                            </div>
                        </div>

                        <div class="col-12 text-right mt-3">
                            <button type="button" class="au-btn au-btn-icon au-btn--green au-btn--small"
                                data-dismiss="modal" aria-label="Close" id="ManipulateServe"
                                onclick="ManipulateServe()">Update</button>
                        </div>
    ...
    <script>
        ...
        async function fetchServerEligibility() {
            try {
                const response = await fetch(`${BASE_URL}/server/launch-server-eligibility`, requestOptions);
                ...
                if (data.eligible) {
                    ...
                    switchLabel.classList.remove("switch-danger");
                    switchLabel.classList.add("switch-success");
                    showAlert("You're eligible for server launch!" , 'success');
                } else {
                    checkbox.checked = false;
                    switchLabel.classList.remove("switch-success");
                    switchLabel.classList.add("switch-danger");
                    showAlert("You're not eligible for server launch!" , 'error');
                }
            } catch (error) {
